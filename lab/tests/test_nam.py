...
def test_write_generic_full_rig_nam_manifest(tmp_path: Path) -> None:
    pack = tmp_path / "NoneStar"
    pack.mkdir()
    (pack / "Mesa Boogie Lone Star - Mateus Asato.nam").write_text(
        json.dumps(
            {
                "version": "0.7.0",
                "architecture": "SlimmableContainer",
                "sample_rate": 48000.0,
                "metadata": {
                    "name": "Mesa Boogie Lone Star - Mateus Asato",
                    "modeled_by": "estetisu",
                    "gear_make": "Mesa Boogie Lone Star",
                    "gear_model": "Mesa Boogie Lone Star",
                    "gear_type": "amp_cab",
                    "tone_type": "clean",
                    "loudness": -18.9,
                    "gain": 0.546,
    ...
    manifest = write_nam_pack_manifest(
        pack,
        output,
        tone_url="https://www.tone3000.com/tones/mesa-boogie-lone-star-mateus-asato-69546",
        reference_id="none-star-mateus-asato-69546",
        tone_id="69546",
        gear_type="full-rig",
        includes_cab=True,
        ir_policy="full-rig-embedded-cab",
    )

    assert manifest["reference_id"] == "none-star-mateus-asato-69546"
    assert manifest["model_policy"]["gear_type"] == "full-rig"
    assert manifest["model_policy"]["includes_cab"] is True
    assert manifest["model_policy"]["ir_policy"] == "full-rig-embedded-cab"
    assert manifest["models"][0]["priority"] is True
    assert manifest["models"][0]["semantics"]["gear_type"] == "amp_cab"
    assert manifest["models"][0]["semantics"]["tone_type"] == "clean"
