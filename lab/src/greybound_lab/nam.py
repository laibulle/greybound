...
def inspect_nam_pack(
    pack_dir: Path,
    *,
    tone_url: str,
    reference_id: str = "ac30hwh-6580",
    tone_id: str | None = "6580",
    gear_type: str = "amp-head",
    includes_cab: bool = False,
    ir_policy: str = "amp-head-no-ir",
    gear_notes: str | None = None,
    priority_models: set[str] | None = None,
) -> dict[str, Any]:
    if priority_models is None:
        priority_models = set(PRIORITY_MODELS) if reference_id == "ac30hwh-6580" else set()
    raw_models = [
        inspect_nam_model(path, reference_id=reference_id)
        for path in sorted(pack_dir.glob("*.nam"))
    ]
    priority_names = priority_models or {model.name for model in raw_models}
    models = [
        NamModelSummary(
            name=model.name,
            ...
            priority=model.name in priority_names,
        )
...
def inspect_nam_model(path: Path, *, reference_id: str = "ac30hwh-6580") -> NamModelSummary:
    ...
    return NamModelSummary(
        ...
        metadata=metadata,
        semantics=parse_nam_model_semantics(name, metadata=metadata, reference_id=reference_id),
        priority=False,
    )


def parse_nam_model_semantics(name: str, *, metadata: dict[str, Any], reference_id: str) -> dict[str, Any]:
    if reference_id == "ac30hwh-6580":
        return parse_ac30hwh_model_name(name)
    semantics: dict[str, Any] = {"raw_name": name}
    for key in ("tone_type", "gear_type", "gear_make", "gear_model"):
