...
from greybound_lab.spice import FIXTURES, klon_centaur_metrics, parse_wrdata, run_spice_fixture, write_spice_dataset
from greybound_lab.stimuli import generate_stimuli
from greybound_lab.metrics import linear_to_db
...
def main() -> None:
    ...
    spice.add_argument("--fixture", required=True, choices=sorted(FIXTURES))
    ...
    integrated_cell.add_argument("--stage", "--component", dest="stage", default="nox30.first_stage")
    ...
    nam.add_argument("--reference-id", default="ac30hwh-6580")
    nam.add_argument("--tone-id", default="6580")
    nam.add_argument("--gear-type", default="amp-head")
    nam.add_argument("--includes-cab", action="store_true")
    nam.add_argument("--ir-policy", default="amp-head-no-ir")
    nam.add_argument("--gear-notes")
    nam.add_argument(
        "--priority-model",
        action="append",
        help="Repeatable NAM model stem to mark as priority. Defaults to AC30 presets or all generic models.",
    )
    ...
    minotaur_nodes = subparsers.add_parser(
        "minotaur-node-compare",
        help="Compare Rust Minotaur per-node diagnostics against the Klon SPICE fixture.",
    )
    minotaur_nodes.add_argument("--rust-report", required=True, type=Path)
    minotaur_nodes.add_argument("--spice-data", type=Path, default=Path("lab/references/spice/klon-centaur.dat"))
    minotaur_nodes.add_argument("--report", required=True, type=Path)
    minotaur_nodes.add_argument("--json", type=Path)
    ...
    elif args.command == "minotaur-node-compare":
        run_minotaur_node_compare(args)
...
def run_inspect_nam_pack(args: argparse.Namespace) -> None:
    manifest = write_nam_pack_manifest(
        ...
        tone_id=args.tone_id,
        gear_type=args.gear_type,
        includes_cab=args.includes_cab,
        ir_policy=args.ir_policy,
        gear_notes=args.gear_notes,
        priority_models=set(args.priority_model) if args.priority_model else None,
    )
...
def run_minotaur_node_compare(args: argparse.Namespace) -> None:
    rust = json.loads(args.rust_report.read_text(encoding="utf-8"))
    fixture = FIXTURES["klon-centaur"]
    spice_trace = parse_wrdata(args.spice_data, fixture.signals)
    spice_metrics = klon_centaur_metrics(spice_trace)
    spice_rms = {
        "input": spice_metrics.input_rms_v,
        "buffer": spice_metrics.buffer_rms_v,
        "clean": spice_metrics.clean_rms_v,
        "drive": spice_metrics.drive_rms_v,
        "clip": spice_metrics.clip_rms_v,
        "mix": spice_metrics.mix_rms_v,
        "tone": spice_metrics.tone_rms_v,
        "output": spice_metrics.output_rms_v,
    }
    rust_nodes = rust.get("nodes", {})
    rust_rms = {
        name: float(values["rms_v"])
        for name, values in rust_nodes.items()
        if isinstance(values, dict) and "rms_v" in values
    }
    rows = []
    rust_input = max(rust_rms.get("input", rust_rms.get("source_input", 0.0)), 1.0e-12)
    spice_input = max(spice_rms["input"], 1.0e-12)
    for name in fixture.signals:
        rust_value = rust_rms.get(name, 0.0)
        spice_value = spice_rms[name]
        rows.append(
            {
                "node": name,
                "rust_rms_v": rust_value,
                "spice_rms_v": spice_value,
                "delta_db": linear_to_db(rust_value / max(spice_value, 1.0e-12)),
                "rust_gain_db": linear_to_db(rust_value / rust_input),
                "spice_gain_db": linear_to_db(spice_value / spice_input),
            }
        )

    worst = max(rows, key=lambda row: abs(row["delta_db"]))
    args.report.parent.mkdir(parents=True, exist_ok=True)
    args.report.write_text(
        minotaur_node_compare_markdown(
            rust_report=args.rust_report,
            spice_data=args.spice_data,
            rust=rust,
            rows=rows,
            worst=worst,
    ...
    if args.json:
        ...
        args.json.write_text(json.dumps({"rows": rows, "worst": worst}, indent=2) + "\n", encoding="utf-8")
        print(f"wrote {args.json}")
    print(f"wrote {args.report}")
    print(f"worst {worst['node']} delta={worst['delta_db']:.2f} dB")


def minotaur_node_compare_markdown(
    *,
    rust_report: Path,
    spice_data: Path,
    rust: dict,
    ...
    controls = rust.get("controls", {})
    stimulus = rust.get("stimulus", {})
    lines = [
        "# Minotaur Node Compare",
        "",
        "Rust node diagnostics compared against the project-owned Klon SPICE fixture.",
        "",
        "## Inputs",
        "",
        f"- Rust report: `{rust_report}`",
        f"- SPICE data: `{spice_data}`",
        f"- Stimulus: {stimulus.get('frequency_hz', '?')} Hz, {stimulus.get('amplitude_v', '?')} V peak",
        (
            "- Controls: "
            f"gain={controls.get('gain', '?')}, "
            f"treble={controls.get('treble', '?')}, "
            f"output={controls.get('output', '?')}"
        ),
        f"- Neural cells disabled: {rust.get('neural_cells_disabled', '?')}",
        "",
        "## Node Levels",
        "",
        "| Node | Rust RMS V | SPICE RMS V | Rust - SPICE | Rust rel. input | SPICE rel. input |",
        "| --- | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in rows:
        lines.append(
            f"| {row['node']} | {row['rust_rms_v']:.9f} | {row['spice_rms_v']:.9f} | "
            f"{row['delta_db']:+.2f} dB | {row['rust_gain_db']:+.2f} dB | "
            f"{row['spice_gain_db']:+.2f} dB |"
        )
    lines.extend(
        [
            "",
            ...
            "",
            (
                f"The largest absolute node-level mismatch is `{worst['node']}` "
                f"at {worst['delta_db']:+.2f} dB. Treat this as a component-level "
                "triage pointer, not a final voicing decision."
            ),
            "",
        ]
    )
