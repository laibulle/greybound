...
The first transparent boundary path is the cathode-follower to Top Boost tone
stack connection: experimental preamp processing now wraps the follower output
in a `ComponentSignal` carrying voltage, source impedance, load impedance,
coupling, DC offset, and headroom before calling the tone stack.

For lab-only boundary experiments, `nox30-experimental` accepts query-style
model parameters. The default form remains transparent:

```json5
amp: {
  model: "nox30-experimental",
}
```
...
```json5
amp: {
  model: "nox30-experimental?tone_source_ohms=47000&tone_load_ohms=47000",
}
```
...
controls. The base desktop `AMP EXPERIMENT` path still loads plain
`nox30-experimental`.
