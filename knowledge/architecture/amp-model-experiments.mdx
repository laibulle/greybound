...
The first `nox30-experimental` alias is intentionally bit-equivalent to
`nox30`. That gives the lab a zero-difference baseline before any real
experimental divergence is introduced.
...
4. **Experimental boundary runner**
   ...
   states in the documented order. Initially it may wrap the existing stages.
