...
explicit stage-boundary behavior. As of 2026-07-02, the formerly
...
- Let a short-lived development target carry circuit-description,
  stage-boundary, and shared-state experiments.
...
stack through an explicit `StageSignal`.
...
- `latency_samples`: local latency contributed by the stage boundary.
...
boundaries already documented by `NOX30_STAGE_BOUNDARIES`:
...
4. **Experimental boundary runner**
   Introduce an experimental runner that processes explicit stage boundary
