...
7. Inspect the visual notebook when metric tables are not enough.
8. Read the generated Markdown report to decide what to investigate next.
...
## Visual WAV Dashboard

Use the notebook dashboard when a render is musically plausible but the next
modeling move is unclear:

```sh
uv --project lab run --with jupyter jupyter lab lab/notebooks/wav-comparison-dashboard.ipynb
```

The notebook calls `greybound_lab.notebook`, which reuses the same WAV loading,
latency alignment, gain correction, segment parsing, and comparison metrics as
`compare-wav`. The first version plots aligned waveform overlays, smoothed
envelopes, post-alignment residual, Welch spectra, residual spectrograms,
global spectral/dynamics bars, and segment-level band residuals. Treat these
plots as investigation views, not as a promotion gate by themselves: final DSP
changes still need fixed test conditions, monitor health, and metric deltas
against the chosen comparison target.
