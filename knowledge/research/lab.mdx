...
The first Nox30 grid sweep against `TopBoost-Gain5` found the lowest
log-spectral distance around `volume = 0.760`, `drive = 0.800`, and
`sag = 0.550`. Treat that as a coarse anchor, not as a final calibration: the
null residual and envelope metrics still indicate dynamic mismatch that needs a
broader sweep or targeted greybox work.
