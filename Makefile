...
NAM_INPUT_DB ?= -70
NAM_OUTPUT_DB ?= -12
NONE_STAR_NAM_PACK_DIR ?= lab/references/nam/NoneStar
NONE_STAR_NAM_PACK_MANIFEST ?= lab/references/nam/manifests/none-star-mateus-asato-69546.json
NONE_STAR_NAM_TONE_URL ?= https://www.tone3000.com/tones/mesa-boogie-lone-star-mateus-asato-69546
NONE_STAR_NAM_MODEL ?= lab/references/nam/NoneStar/Mesa Boogie Lone Star - Mateus Asato.nam
NONE_STAR_NAM_OUTPUT_WAV ?= lab/references/nam/renders/none-star-mateus-asato.wav
...
LANDING_VERCEL_BUILD_ARGS ?= $(VERCEL_BUILD_ARGS)
...
desktop:
	cargo run -p greybound-free

desktop-release:
	cargo run -p greybound-free --release

run-desktop: desktop-release
	$(DESKTOP)

desktop-package:
	cargo build -p greybound-free --release
	...
	@{ \
		...
		printf '%s\n' '  <string>Greybound Free needs microphone access for live guitar input.</string>'; \
		printf '%s\n' '</dict>'; \
		printf '%s\n' '</plist>'; \
...
lab-inspect-none-star-nam:
	uv --project lab run greybound-lab inspect-nam-pack \
		--pack-dir "$(NONE_STAR_NAM_PACK_DIR)" \
		--manifest "$(NONE_STAR_NAM_PACK_MANIFEST)" \
		--tone-url "$(NONE_STAR_NAM_TONE_URL)" \
		--reference-id "none-star-mateus-asato-69546" \
		--tone-id "69546" \
		--gear-type "full-rig" \
		--includes-cab \
...
lab-render-none-star-nam:
	@test -n "$(NAM_RENDERER)" || (echo "NAM_RENDERER is required. It must accept placeholders: {model}, {input_wav}, {output_wav}, {sample_rate}, {render_seconds}, {ir_wav}." >&2; exit 2)
	uv --project lab run greybound-lab render-nam \
		--model "$(NONE_STAR_NAM_MODEL)" \
		--input-wav "$(NAM_INPUT_WAV)" \
		--output-wav "$(NONE_STAR_NAM_OUTPUT_WAV)" \
		--metadata "$(NONE_STAR_NAM_METADATA)" \
		--renderer-command "$(NAM_RENDERER)" \
		--sample-rate "$(NAM_SAMPLE_RATE)" \
		--render-seconds "$(NAM_RENDER_SECONDS)" \
...
lab-spice-none-star-tone-presence:
	uv --project lab run greybound-lab spice-run \
...
vercel-build: landing-vercel-build

landing-deploy: landing-vercel-build
	cd landing && $(VERCEL) deploy $(LANDING_VERCEL_DEPLOY_ARGS)
