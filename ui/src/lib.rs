...
impl Boutique59Ui {
    ...
    pub fn view(&self) -> Element<'_, Message> {
        ...
        let mid = (self.devices.len() + 1) / 2;
        let (first_half, second_half) = self.devices.split_at(mid);

        let pedalboard_row1 = first_half.iter().enumerate().fold(
        ...
        let pedalboard_row2 = second_half.iter().enumerate().fold(
            row![].spacing(14).align_items(Alignment::Center),
            |row, (index, device)| {
                let actual_index = index + mid;
                row.push(self.render_pedal_module(
                    actual_index,
                    device,
        ...
        let pedalboard = container(column![pedalboard_row1, pedalboard_row2].spacing(14))
            .style(skeuo_container(Color::from_rgb(0.12, 0.10, 0.08)))
            .padding(10)
            .width(Length::Fill);
