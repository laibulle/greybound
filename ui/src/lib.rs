...
const PEDAL_STANDARD_SOURCE_HEIGHT: f32 = 2172.0;
...
/// The model-owned input to the pedal asset compiler.
///
/// This deliberately describes only the non-mechanical enclosure artwork. The
/// renderer owns every placement-sensitive part (labels, knobs, LEDs,
/// footswitch and jacks) so an image model can never move, omit, or obscure a
/// functional control.
#[derive(Debug, Clone, Copy)]
pub struct PedalAssetGenerationSpec {
    /// Bump when either the visual or mechanical contract changes.
    pub recipe_version: u32,
    /// Human-facing wordmark that may be part of the enclosure art.
    pub model_name: &'static str,
    /// Project-owned source destination for a newly approved empty enclosure.
    pub faceplate_path: &'static str,
    /// The visual identity supplied to the image model; geometry is supplied
    /// separately by the generated construction guide.
...
pub struct ModelRenderSpec {
    ...
    /// Present only for pedal faceplates that can be regenerated by the
    /// deterministic asset pipeline.
    pub generation: Option<PedalAssetGenerationSpec>,
}
...
pub const MINOTAUR_PEDAL_GENERATION: PedalAssetGenerationSpec = PedalAssetGenerationSpec {
    recipe_version: 1,
    model_name: "MINOTAUR",
    faceplate_path: "assets/pedals/minotaur@4x.png",
    art_direction: "warm antique gold anodized aluminium, restrained sunburst engraving, dark umber ink, premium but roadworthy",
};

pub const NAM_PEDAL_GENERATION: PedalAssetGenerationSpec = PedalAssetGenerationSpec {
    recipe_version: 1,
    model_name: "NAM PEDAL",
    faceplate_path: "assets/pedals/nam-pedal@4x.png",
    art_direction: "deep graphite powder-coated enclosure, restrained cyan technical accents, modern neural-audio laboratory instrument",
};

pub const MONARCH_PEDAL_GENERATION: PedalAssetGenerationSpec = PedalAssetGenerationSpec {
    recipe_version: 1,
    model_name: "MONARCH",
    faceplate_path: "assets/pedals/monarch@4x.png",
    art_direction: "midnight navy enamel over machined metal, subtle brass line work, ceremonial but understated boutique overdrive",
};

pub const MUFFIN_PEDAL_GENERATION: PedalAssetGenerationSpec = PedalAssetGenerationSpec {
    recipe_version: 1,
    model_name: "MUFFIN",
    faceplate_path: "assets/pedals/muffin@4x.png",
    art_direction: "1970s graphite-black cast aluminium, aged cream screen-print texture, dark brass accents, rugged fuzz-box character",
};

pub const LUMEN_PEDAL_GENERATION: PedalAssetGenerationSpec = PedalAssetGenerationSpec {
    recipe_version: 1,
    model_name: "LUMEN",
    faceplate_path: "assets/pedals/lumen@4x.png",
    art_direction: "warm pearl enamel and champagne metal, restrained optical-compressor laboratory aesthetic, fine amber accent lines",
};

pub const SPRINGFIELD_PEDAL_GENERATION: PedalAssetGenerationSpec = PedalAssetGenerationSpec {
    recipe_version: 1,
    model_name: "SPRINGFIELD",
    faceplate_path: "assets/pedals/springfield@4x.png",
    art_direction: "seafoam enamel, nickel hardware palette, restrained 1960s spring-reverb amplifier character, subtle waterline texture",
};

pub const AURALITH_PEDAL_GENERATION: PedalAssetGenerationSpec = PedalAssetGenerationSpec {
    recipe_version: 1,
    model_name: "AURALITH",
    faceplate_path: "assets/pedals/auralith@4x.png",
    art_direction: "dark iridescent mineral lacquer with embedded crystal flecks, selective glossy clear-coat, premium modern ambient effect",
};
...
pub const MINOTAUR_PEDAL_RENDER_SPEC: ModelRenderSpec = ModelRenderSpec {
    ...
    generation: Some(MINOTAUR_PEDAL_GENERATION),
};
...
pub const NAM_PEDAL_RENDER_SPEC: ModelRenderSpec = ModelRenderSpec {
    ...
    generation: Some(NAM_PEDAL_GENERATION),
};
...
pub const MONARCH_PEDAL_RENDER_SPEC: ModelRenderSpec = ModelRenderSpec {
    ...
    generation: Some(MONARCH_PEDAL_GENERATION),
};
...
pub const MUFFIN_PEDAL_RENDER_SPEC: ModelRenderSpec = ModelRenderSpec {
    ...
    generation: Some(MUFFIN_PEDAL_GENERATION),
};
...
pub const LUMEN_PEDAL_RENDER_SPEC: ModelRenderSpec = ModelRenderSpec {
    ...
    generation: Some(LUMEN_PEDAL_GENERATION),
};
...
pub const SPRINGFIELD_PEDAL_RENDER_SPEC: ModelRenderSpec = ModelRenderSpec {
    ...
    generation: Some(SPRINGFIELD_PEDAL_GENERATION),
};
...
pub const AURALITH_PEDAL_RENDER_SPEC: ModelRenderSpec = ModelRenderSpec {
    ...
    generation: Some(AURALITH_PEDAL_GENERATION),
};
...
fn render_control_label_baseline(control: &RenderControlSpec, origin: Point, size: Size) -> f32 {
    const STANDARD_SOURCE_OFFSET: f32 = 177.0;
    const SOURCE_PER_LOGICAL_PIXEL: f32 = 4.0;
    const LABEL_CLEARANCE_LOGICAL: f32 = 20.0;

    let center = render_control_center(control, origin, size);
    let source_offset = STANDARD_SOURCE_OFFSET.max(
        control.radius * SOURCE_PER_LOGICAL_PIXEL
            + LABEL_CLEARANCE_LOGICAL * SOURCE_PER_LOGICAL_PIXEL,
    );
    match control.widget {
        // Vertical selectors are immediately above the footswitch. Put their
        // label above the travel instead of letting it compete for the same
        // narrow lower-faceplate space.
        RenderControlWidget::Slider => {
            center.y - size.height * STANDARD_SOURCE_OFFSET / PEDAL_STANDARD_SOURCE_HEIGHT
        }
        _ => center.y + size.height * source_offset / PEDAL_STANDARD_SOURCE_HEIGHT,
    }
}
...
fn draw_pedal(
    ...
    let uses_generated_faceplate = uses_asset
        && render_spec.generation.is_some()
        && render_spec.typography == RenderTypographyPolicy::DrawnByUi;
    ...
    // Legacy full-faceplate PNGs already contain their side jacks. Empty
    // faceplates promoted through PedalAssetGenerationSpec opt into this
    // deterministic chassis layer with DrawnByUi typography.
    if !uses_asset || uses_generated_faceplate {
        draw_side_jack(
            frame,
        ...
        if control.widget == RenderControlWidget::Slider {
            if device.model == DeviceModel::Muffin {
                draw_muffin_vertical_selector(
                    frame,
                    render_control_center(control, origin, size),
                    render_control_label_baseline(control, origin, size),
                    control.label,
                    kind,
...
fn draw_muffin_vertical_selector(
    ...
    draw_text(
        frame,
        label,
        Point::new(center.x, label_baseline_y),
        label_size,
        label_color,
        Horizontal::Center,
    );
...
mod tests {
    ...
    #[test]
    fn muffin_selector_labels_clear_the_footswitch() {
        let origin = Point::ORIGIN;
        let source_size = Size::new(1200.0, PEDAL_STANDARD_SOURCE_HEIGHT);
        let footswitch = MUFFIN_PEDAL_CONTROLS
            .iter()
            .find(|control| control.widget == RenderControlWidget::Footswitch)
            .expect("Muffin requires a footswitch");
        let footswitch_center = render_control_center(footswitch, origin, source_size);
        let footswitch_top = footswitch_center.y - footswitch.radius * 4.0;

        for control in MUFFIN_PEDAL_CONTROLS
            .iter()
            .filter(|control| control.widget == RenderControlWidget::Slider)
        {
            let baseline = render_control_label_baseline(control, origin, source_size);
            assert!(
                baseline + 16.0 < footswitch_top,
                "{} must remain above the footswitch",
                control.label
            );
        }
    }
    ...
    #[test]
    fn large_knob_labels_clear_their_knob_body() {
        let origin = Point::ORIGIN;
        let source_size = Size::new(1200.0, PEDAL_STANDARD_SOURCE_HEIGHT);
        let dwell = SPRINGFIELD_PEDAL_CONTROLS
            .iter()
            .find(|control| control.label == "Dwell")
            .expect("Springfield requires a Dwell control");
        let center = render_control_center(dwell, origin, source_size);
        let baseline = render_control_label_baseline(dwell, origin, source_size);

        assert!(baseline - 32.0 > center.y + dwell.radius * 4.0);
    }
