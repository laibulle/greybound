...
fn main() -> Result<()> {
    ...
    let params = Arc::new(args.initial_params);
    let audio_params = params.clone();
    let audio_args = Args {
        initial_params: VoxBoxParams::default(), // Dummy, not used in audio thread
        ..args
    };

    // Start audio thread
    thread::spawn(move || {
        if let Err(e) = run_audio(host, audio_args, audio_params) {
    ...
    // Run Iced UI
    iced::application(
        move || StandaloneApp::new(params.clone()),
...
fn run_audio(host: cpal::Host, args: Args, params: Arc<VoxBoxParams>) -> Result<()> {
    ...
    for &channel in &args.output_channels {
...
fn parse_args(host: &cpal::Host) -> Result<Args> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--output-db" => {
                let db = next_value(&mut args, "--output-db")?.parse::<f32>()?;
                unsafe { initial_params.master._internal_set_plain_value(nih_plug::prelude::util::db_to_gain(db)) };
            }
            "--ir" => {
                unsafe { initial_params.speaker_ir._internal_set_plain_value(true) };
            }
            "--list-devices" => {
                print_devices(host)?;
                std::process::exit(0);
    ...
    Ok(Args {
        input_device: input_device.unwrap_or_else(|| host.default_input_device().map(|d| d.name().unwrap()).unwrap_or_default()),
        output_device: output_device.unwrap_or_else(|| host.default_output_device().map(|d| d.name().unwrap()).unwrap_or_default()),
