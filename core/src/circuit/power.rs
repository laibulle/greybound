...
pub struct PushPull6L6Params {
    ...
    pub supply_resistance: f32,
    pub supply_capacitance: f32,
    pub fixed_bias_voltage: f32,
    pub bias_shift_resistance: f32,
    pub bias_shift_capacitance: f32,
    pub idle_current: f32,
    pub drive_gain: f32,
    pub current_gain: f32,
    pub load_current_coupling: f32,
...
pub struct PushPull6L6Stage {
    params: PushPull6L6Params,
    supply_voltage: f32,
    screen_voltage: f32,
    bias_shift_voltage: f32,
    plate_a_voltage: f32,
    plate_b_voltage: f32,
    reference_plate_a_voltage: f32,
    reference_plate_b_voltage: f32,
...
#[derive(Clone, Copy, Debug)]
pub struct PushPull6L6OperatingPoint {
    pub supply_voltage: f32,
    pub screen_voltage: f32,
    pub plate_a_voltage: f32,
    pub plate_b_voltage: f32,
    pub fixed_bias_voltage: f32,
    pub bias_shift_voltage: f32,
    pub positive_current: f32,
    pub negative_current: f32,
    pub positive_load_current: f32,
    pub negative_load_current: f32,
...
impl PushPull6L6Stage {
    pub fn new(params: PushPull6L6Params) -> Self {
        let idle_plate_drop = params.idle_current * 0.5 * params.primary_half_resistance;
        let idle_plate = params.nominal_supply_voltage - idle_plate_drop;
        let mut stage = Self {
            params,
            ...
            positive_load_current: 0.0,
            negative_load_current: 0.0,
            positive_screen_current: params.idle_current * 0.032,
            negative_screen_current: params.idle_current * 0.032,
            attack_current: 0.0,
            attack_reference_current: params.idle_current,
        };
        for _ in 0..512 {
    ...
    pub fn operating_point(&self) -> PushPull6L6OperatingPoint {
        PushPull6L6OperatingPoint {
            supply_voltage: self.supply_voltage,
            screen_voltage: self.screen_voltage,
            plate_a_voltage: self.plate_a_voltage,
            plate_b_voltage: self.plate_b_voltage,
            fixed_bias_voltage: self.params.fixed_bias_voltage,
            bias_shift_voltage: self.bias_shift_voltage,
            positive_current: self.positive_current,
            negative_current: self.negative_current,
            positive_load_current: self.positive_load_current,
            negative_load_current: self.negative_load_current,
    ...
    pub fn process(&mut self, drive: f32, sag: f32) -> f32 {
        let supply_ratio = self.supply_ratio();
        let drive_voltage = drive * self.params.drive_gain * supply_ratio;
        let bias_offset = self.bias_shift_voltage * 0.30;
        ...
        self.update_attack_current(total_plate_current);
        let attack_current = self.attack_current * self.params.attack_current_coupling;
        let total_current = total_plate_current + total_screen_current + attack_current;
        ...
        self.positive_screen_current = positive_screen_current;
        self.negative_screen_current = negative_screen_current;
        self.update_bias_shift(total_current);
        self.update_screen(total_screen_current + attack_current * 0.12);
    ...
    fn reflected_load_current(
        ...
        let load_current =
            differential_voltage / (self.params.primary_half_resistance * 2.0).max(1.0);
        let stress = (drive_voltage.abs() / 18.0).clamp(0.0, 1.0);
        let coupled_current = load_current * self.params.load_current_coupling * stress;

        if drive_voltage >= 0.0 {
            (coupled_current, coupled_current * 0.12)
        } else {
            (coupled_current * 0.12, coupled_current)
        }
    }

    fn update_attack_current(&mut self, total_plate_current: f32) {
        let idle_plate_current = self.params.idle_current;
        let reference_coefficient = 1.0 - (-1.0 / (self.params.sample_rate * 0.120)).exp();
        self.attack_reference_current +=
            reference_coefficient * (total_plate_current - self.attack_reference_current);

        let excess_current = (total_plate_current - idle_plate_current * 0.92).max(0.0);
        let excess_target = excess_current * excess_current * 0.55 / idle_plate_current.max(1e-6);
        let transient_current =
            (total_plate_current - self.attack_reference_current * 1.08).max(0.0);
        let transient_target = transient_current * 0.72;
        let target = excess_target.max(transient_target);
        let time_constant = if target > self.attack_current {
            0.006
        } else {
        ...
        let coefficient = 1.0 - (-1.0 / (self.params.sample_rate * time_constant)).exp();
        self.attack_current += coefficient * (target - self.attack_current);
        self.attack_current = self.attack_current.clamp(0.0, idle_plate_current * 0.50);
    }
    ...
    fn pentode_point(&self, plate_voltage: f32, grid_drive: f32) -> PentodePoint {
        let plate_to_cathode = plate_voltage.max(0.0);
        let screen_to_cathode = self.screen_voltage.min(self.supply_voltage).max(0.0);
        let grid_to_cathode = grid_drive - self.params.fixed_bias_voltage;
        let control = softplus(grid_to_cathode + screen_to_cathode / 11.0, 0.85);
        let saturation = 1.0 - (-plate_to_cathode / 78.0).exp();
        let d_saturation_d_plate = (-plate_to_cathode / 78.0).exp() / 78.0;
        let screen_factor =
            (screen_to_cathode / self.params.screen_voltage.max(1.0)).clamp(0.0, 1.18);
        let shaped = self.params.current_gain * control.powf(1.22) * screen_factor
            / (1.0 + control * self.params.compression);
        let low_plate_screen_pull = (1.0 - plate_to_cathode / screen_to_cathode.max(1.0))
            .clamp(0.0, 1.0)
            .powi(2);
        let screen_current = shaped
            * (0.035 + 0.085 * low_plate_screen_pull)
            * (0.58 + 0.42 * saturation)
            * screen_factor;

        PentodePoint {
            current: (shaped * saturation).clamp(0.0, 0.150),
            screen_current: screen_current.clamp(0.0, 0.024),
            d_current_d_plate: (shaped * d_saturation_d_plate).max(0.0),
        }
    }
    ...
    fn update_screen(&mut self, total_screen_current: f32) {
        ...
        self.screen_voltage += coefficient * (target - self.screen_voltage);
        self.screen_voltage = self.screen_voltage.clamp(
            self.params.screen_voltage * 0.50,
            self.params.screen_voltage.min(self.supply_voltage),
        );
    }

    fn update_supply(&mut self, total_current: f32, sag: f32) {
        let effective_current = total_current * (0.08 + sag.clamp(0.0, 1.0) * 0.58);
        let target =
            self.params.nominal_supply_voltage - effective_current * self.params.supply_resistance;
        let coefficient = 1.0
            - (-1.0
        ...
        self.supply_voltage += coefficient * (target - self.supply_voltage);
        self.supply_voltage = self.supply_voltage.clamp(
            self.params.nominal_supply_voltage * 0.55,
            self.params.nominal_supply_voltage,
        );
    }

    fn update_bias_shift(&mut self, total_current: f32) {
        let excess_current = (total_current - self.params.idle_current * 0.96).max(0.0);
        let target = excess_current * self.params.bias_shift_resistance;
        let coefficient = 1.0
            - (-1.0
                / (self.params.sample_rate
                    * self.params.bias_shift_resistance
                    * self.params.bias_shift_capacitance))
                .exp();
        self.bias_shift_voltage += coefficient * (target - self.bias_shift_voltage);
        self.bias_shift_voltage = self
            .bias_shift_voltage
            .clamp(0.0, self.params.fixed_bias_voltage * 0.18);
    }

    fn supply_ratio(&self) -> f32 {
        (self.supply_voltage / self.params.nominal_supply_voltage).clamp(0.55, 1.05)
    }
}
...
mod tests {
    ...
    fn stage_6l6() -> PushPull6L6Stage {
        PushPull6L6Stage::new(PushPull6L6Params {
            sample_rate: 48_000.0,
            ...
            bias_shift_resistance: 42_000.0,
            bias_shift_capacitance: 2.2e-6,
            idle_current: 0.070,
            drive_gain: 24.0,
            current_gain: 0.018,
            load_current_coupling: 0.95,
            attack_current_coupling: 0.42,
            compression: 0.16,
            output_scale: 0.010,
        })
    }
    ...
    #[test]
    fn six_l6_silence_stays_centered_and_finite() {
        let mut stage = stage_6l6();
        for _ in 0..2048 {
            let output = stage.process(0.0, 0.5);
            assert!(output.is_finite());
            assert!(output.abs() < 1e-5, "output={output}");
    ...
    #[test]
    fn six_l6_drive_moves_screen_and_bias_state() {
        let mut stage = stage_6l6();
        for _ in 0..48_000 {
            stage.process(0.0, 0.4);
        }
        let idle = stage.operating_point();
        let mut min_screen = idle.screen_voltage;
        let mut max_bias_shift = idle.bias_shift_voltage;

        for sample_idx in 0..24_000 {
            let input = (std::f32::consts::TAU * 110.0 * sample_idx as f32 / 48_000.0).sin() * 0.95;
            stage.process(input, 0.65);
            let op = stage.operating_point();
            min_screen = f32::min(min_screen, op.screen_voltage);
            max_bias_shift = f32::max(max_bias_shift, op.bias_shift_voltage);
        }

        assert!(
            min_screen < idle.screen_voltage - 0.4,
            "idle={idle:?}, min_screen={min_screen}"
        );
        assert!(
            max_bias_shift > idle.bias_shift_voltage + 0.04,
            "idle={idle:?}, max_bias_shift={max_bias_shift}"
        );
    }
