...
mod tests {
    ...
    #[test]
    fn parses_studiodelay_controls() {
        let rig = RigConfig::from_json5(
            r#"
            {
              name: 'unit-test-rig',
        ...
        assert_eq!(
            chain.fx_loop,
            vec![DeviceSlotConfig::bypassed(DeviceConfig::StudioDelay)]
        );
        assert!(matches!(
            controls[0].controls,
            DeviceControls::StudioDelay(StudioDelayControls {
                ...
                mod_depth,
                mix,
            }) if (time_ms - 420.0).abs() < 1e-6
                && (feedback - 0.41).abs() < 1e-6
                && (tone - 0.62).abs() < 1e-6
                && (mod_depth - 0.09).abs() < 1e-6
                && (mix - 0.21).abs() < 1e-6
        ));
        assert!(controls[0].bypassed);
    }
