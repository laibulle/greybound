...
mod monitor_log;
mod neural_overrides;
mod wav_io;

use greybound::amp::{AmpControls, NeuralCellMode, Nox30OperatingPoint};
...
use neural_overrides::{
    apply_neural_overrides, neural_cell_mode_name, parse_graybox_cell_override,
    parse_neural_cell_mode, parse_neural_cell_override, GrayboxCellOverride, NeuralCellOverride,
};
use ratatui::{
...
use wav_io::{load_wav_input, resample_linear};
...
impl SharedAmpControls {
    fn new(controls: AmpControls, enabled: bool) -> Self {
        Self {
            ...
            master: Arc::new(AtomicU32::new(controls.master.to_bits())),
    ...
    fn load(&self) -> AmpControls {
        AmpControls {
            ...
            master: load_atomic_f32(&self.master),
...
impl SharedDeviceControl {
    fn new(controls: DeviceControls) -> Self {
        match controls {
            ...
            DeviceControls::StudioDelay(controls) => Self::StudioDelay {
                time_ms: AtomicU32::new(controls.time_ms.to_bits()),
                feedback: AtomicU32::new(controls.feedback.to_bits()),
                tone: AtomicU32::new(controls.tone.to_bits()),
    ...
    fn load(&self) -> DeviceControls {
        match self {
            ...
            Self::StudioDelay {
                ...
                mod_depth,
                mix,
            } => DeviceControls::StudioDelay(StudioDelayControls {
                time_ms: load_atomic_f32(time_ms),
                feedback: load_atomic_f32(feedback),
                tone: load_atomic_f32(tone),
    ...
    fn adjust(&self, param_index: usize, delta: f32, min: f32, max: f32) {
        match self {
            ...
            Self::StudioDelay {
                ...
                [time_ms, feedback, tone, mod_depth, mix]
                    .get(param_index)
                    .copied(),
                delta,
                min,
...
fn default_device_controls(device: DeviceConfig) -> DeviceControls {
    match device {
        ...
        DeviceConfig::StudioDelay => DeviceControls::StudioDelay(StudioDelayControls::default()),
...
fn device_control_value(controls: DeviceControls, id: &str) -> ControlValue {
    match controls {
        ...
        DeviceControls::StudioDelay(controls) => match id {
            "time_ms" => ControlValue::Number(controls.time_ms),
            "feedback" => ControlValue::Number(controls.feedback),
            "tone" => ControlValue::Number(controls.tone),
...
fn main() -> Result<()> {
    ...
    if let Some(path) = &args.minotaur_node_report {
    ...
    if !args.neural_cells.is_empty() {
        eprintln!(
            "Neural cells: {} stage/cell target(s), mode {}",
...
fn write_minotaur_node_report(path: &Path, args: &Args) -> Result<()> {
    ...
    let controls = MinotaurControls {
        gain: args.diagnostic_gain.clamp(0.0, 1.0),
        treble: args.diagnostic_treble.clamp(0.0, 1.0),
        output: args.diagnostic_output.clamp(0.0, 1.0),
    };
    let mut minotaur = Minotaur::new(sample_rate);
    let mut source = NodeAccumulator::default();
    let mut input = NodeAccumulator::default();
    let mut buffer = NodeAccumulator::default();
    let mut clean = NodeAccumulator::default();
    let mut drive = NodeAccumulator::default();
    let mut clip = NodeAccumulator::default();
    let mut mix = NodeAccumulator::default();
    let mut tone = NodeAccumulator::default();
    let mut output = NodeAccumulator::default();
    ...
    for sample_index in 0..total_samples {
        ...
        let signal = ElectricalSignal::new(sample, GUITAR_SOURCE_IMPEDANCE_OHMS);
        let (_signal, stages) = minotaur.process_with_node_voltages(signal, controls);
        if sample_index < settle_samples {
            continue;
        }
        record_minotaur_nodes(
            sample,
            stages,
            &mut source,
            &mut input,
    ...
    let mut file = File::create(path)
        .with_context(|| format!("could not create Minotaur node report '{}'", path.display()))?;
    writeln!(file, "{{")?;
    writeln!(file, "  \"schema_version\": 1,")?;
    writeln!(file, "  \"model\": \"minotaur\",")?;
    writeln!(file, "  \"sample_rate_hz\": {},", args.sample_rate)?;
    writeln!(
        file,
        "  \"stimulus\": {{\"kind\": \"sine\", \"frequency_hz\": {:.9}, \"amplitude_v\": {:.9}, \"duration_s\": {:.9}, \"settle_s\": 0.050000000}},",
        args.diagnostic_frequency_hz,
        args.diagnostic_input_v,
        args.diagnostic_seconds
    )?;
    writeln!(
        file,
        "  \"controls\": {{\"gain\": {:.9}, \"treble\": {:.9}, \"output\": {:.9}}},",
        controls.gain, controls.treble, controls.output
    )?;
    writeln!(
        file,
        "  \"neural_cells_disabled\": {},",
        args.disable_neural_cell
    )?;
    writeln!(file, "  \"nodes\": {{")?;
    write_node_json(&mut file, "source_input", source, true)?;
    write_node_json(&mut file, "input", input, true)?;
    write_node_json(&mut file, "buffer", buffer, true)?;
    write_node_json(&mut file, "clean", clean, true)?;
    write_node_json(&mut file, "drive", drive, true)?;
    write_node_json(&mut file, "clip", clip, true)?;
    write_node_json(&mut file, "mix", mix, true)?;
    write_node_json(&mut file, "tone", tone, true)?;
    write_node_json(&mut file, "output", output, false)?;
    writeln!(file, "  }}")?;
    writeln!(file, "}}")?;
    Ok(())
}
...
fn record_minotaur_nodes(
    ...
    source: &mut NodeAccumulator,
    input: &mut NodeAccumulator,
    buffer: &mut NodeAccumulator,
    clean: &mut NodeAccumulator,
    drive: &mut NodeAccumulator,
    clip: &mut NodeAccumulator,
    mix: &mut NodeAccumulator,
    tone: &mut NodeAccumulator,
    output: &mut NodeAccumulator,
) {
    source.record(source_sample);
    input.record(stages.loaded_input);
    buffer.record(stages.buffered);
    clean.record(stages.clean_feed);
    drive.record(stages.drive_stage);
    clip.record(stages.clipped);
    mix.record(stages.sum_node);
    tone.record(stages.voiced);
    output.record(stages.output);
}

fn write_node_json(
    file: &mut File,
    name: &str,
    stats: NodeAccumulator,
    trailing_comma: bool,
) -> Result<()> {
    let comma = if trailing_comma { "," } else { "" };
    writeln!(
        file,
        "    \"{}\": {{\"rms_v\": {:.12}, \"peak_v\": {:.12}, \"samples\": {}}}{}",
        name,
        stats.rms(),
        stats.peak,
        stats.count,
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--minotaur-node-report" => {
                minotaur_node_report = Some(PathBuf::from(next_value(
                    &mut args,
                    "--minotaur-node-report",
                )?))
            }
            ...
            "--diagnostic-input-v" => {
                diagnostic_input_v = next_value(&mut args, "--diagnostic-input-v")?.parse()?
            }
            "--diagnostic-seconds" => {
                diagnostic_seconds = next_value(&mut args, "--diagnostic-seconds")?.parse()?
            }
            "--diagnostic-gain" => {
                diagnostic_gain = next_value(&mut args, "--diagnostic-gain")?.parse()?
            }
            "--diagnostic-treble" => {
                diagnostic_treble = next_value(&mut args, "--diagnostic-treble")?.parse()?
            }
            "--diagnostic-output" => {
...
fn print_help() {
    eprintln!(
        "Usage: greybound-cli --rig PATH|- [OPTIONS]\n\
         ...
         \x20 --neural-cell STAGE=PATH  Run a neural counterpart for a supported stage/cell: nox30.first_stage, none-star.power_6l6, minotaur.clip, minotaur.tone\n\
         \x20 --graybox-cell STAGE=CONFIG  Run a gray-box counterpart; CONFIG may be 'accepted-live', 'accepted', or a JSON path\n\
...
mod tests {
    ...
    fn loads_sample_wav_input_channel() {
        ...
        ));
        {
            ...
            for sample_idx in 0..128 {
                let sample = (std::f32::consts::TAU * sample_idx as f32 / 32.0).sin() * 0.25;
    ...
    fn parses_neural_cell_override_and_mode() {
        let override_ =
            parse_neural_cell_override("nox30.first_stage=lab/models/cell/model.greybound.json")
                .unwrap();
        assert_eq!(override_.stage, "nox30.first_stage");
        assert_eq!(
            override_.descriptor_path,
            PathBuf::from("lab/models/cell/model.greybound.json")
        );
        let power_override = parse_neural_cell_override(
            "none-star.power_6l6=lab/models/none-star-power-6l6/model.greybound.json",
        )
        .unwrap();
        assert_eq!(power_override.stage, "none-star.power_6l6");
        assert_eq!(
            power_override.descriptor_path,
            PathBuf::from("lab/models/none-star-power-6l6/model.greybound.json")
        );
        assert_eq!(
